        ...
        <div class="third">
            <table>
                <tr>
                    ...
                    <th width="10%">表名</th>
                    <th width="10%">字段名</th>
                    <th width="20%" class="self_th_td">数据ID</th>
                    <th width="20%" class="self_th_td">文件名</th>
                    <th width="10%">上传时间</th>
                ...
                <c:forEach items="${attachments}" var="attachment" varStatus="vs">
                    <tr>
                        ...
                        <td class="self_th_td">${attachment.dataId}</td>
                        <td class="self_th_td">${attachment.srcName}</td>
                        <td>
                            <c:if test="${attachment.createTime > 0}">
                                <jsp:useBean id="createTime" class="java.util.Date"/>
                                <jsp:setProperty name="createTime" property="time" value="${attachment.createTime}"/>
                                <fmt:formatDate value="${createTime}" pattern="yyyy-MM-dd HH:mm"/>
                            </c:if>
                        </td>
                        <td style="text-align: left;">
                            <span><a href="${baseUrl}/web/api/down/file.do?id=${attachment.id}" target="rightFrame"><img src="${baseUrl}/img/chakan.png">下载</a></span>
                        </td>
                    </tr>
                </c:forEach>
            </table>
            <div class="fenye">
                <ul class="fy_ul">
                    ...
                    <c:if test="${currPage > 1}">
                        <li><a href="${baseUrl}/web/common/attachments.do?page=${currPage - 1}">上一页</a></li>
                    </c:if>
                    ...
                    <c:if test="${currPage < totalPage}">
                        <li><a href="${baseUrl}/web/common/attachments.do?page=${currPage + 1}">下一页</a></li>
                    </c:if>
                </ul>
            </div>
        </div>
