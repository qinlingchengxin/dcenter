    ...
    <div class="content_right fr">
        ...
        <div class="second">
            <form id="form">
                ...
                <div>
                    ...
                    <select id="doc-ipt-srcTabName-1" name="srcTabName" onchange="changePk(this.value, 0);">
                        <c:forEach items="${srcTables}" var="srcTable">
                            <option value="${srcTable.name}" <c:if test="${fn:toUpperCase(srcTable.name) == fn:toUpperCase(etlEntity.srcTabName)}">selected="selected"</c:if>>${srcTable.comment}-${srcTable.name}</option>
                        </c:forEach>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="doc-ipt-desTabName-1" name="desTabName" onchange="changePk(this.value, 1);">
                        <c:forEach items="${desTables}" var="desTable">
                            <option value="${desTable.name}" <c:if test="${fn:toUpperCase(desTable.name) == fn:toUpperCase(etlEntity.desTabName)}">selected="selected"</c:if>>${desTable.comment}-${desTable.name}</option>
                        </c:forEach>
                    </select>
                </div>

                <div>
                    <span class="title">原表主键</span>
                    <select id="doc-ipt-srcPrimaryKey-1" name="srcPrimaryKey">
                        <c:forEach items="${srcPks}" var="srcPk">
                ...
                <div>
                    <span class="title">目标表主键</span>
                    <select id="doc-ipt-desPrimaryKey-1" name="desPrimaryKey">
                        <c:forEach items="${desPks}" var="desPk">
