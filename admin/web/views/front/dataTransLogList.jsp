...
<head>
    ...
    <script type="text/javascript">
        function searchData() {
            ...
            window.location = "${baseUrl}/front/dataTransLog/list.do?tableName=" + tableName + "&status=" + status + "&type=${type}&content=" + content + "&startTime=" + startTime + "&endTime=" + endTime;
        }
    </script>
        ...
        <div class="third">
                ...
                <c:forEach items="${dataTransLogs}" var="dataTransLog" varStatus="vs">
                    <tr>
                        ...
                        <td>
                            ...
                            <c:if test="${dataTransLog.status == 0}">成功</c:if>
                        </td>
                        <td class="self_th_td" style="max-width: 100px;">${dataTransLog.content}</td>
                        <c:if test="${dataTransLog.transType == 0}">
                            <td class="self_th_td">${dataTransLog.failedContent}</td>
                        </c:if>
                        <td>
                        ...
                        <td style="text-align: left;">
                            <span><a href="${baseUrl}/front/dataTransLog/dataDetail.do?logId=${dataTransLog.id}" target="rightFrame"><img src="${baseUrl}/img/chakan.png">数据详情</a></span>
                        </td>
                    </tr>
                </c:forEach>
            </table>
            <div class="fenye">
                <ul class="fy_ul">
                    ...
                    <c:if test="${currPage > 1}">
                        <li><a href="${baseUrl}/front/dataTransLog/list.do?page=${currPage - 1}&type=${type}">上一页</a></li>
                    </c:if>
                    ...
                    <c:if test="${currPage < totalPage}">
                        <li><a href="${baseUrl}/front/dataTransLog/list.do?page=${currPage + 1}&type=${type}">下一页</a></li>
                    </c:if>
                </ul>
            </div>
        </div>
