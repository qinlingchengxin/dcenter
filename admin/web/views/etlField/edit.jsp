        ...
        <div class="second">
            <form id="form">
                ...
                <div>
                    ...
                    <select id="doc-ipt-srcFieldName-1" name="srcFieldName">
                        <c:forEach items="${srcFields}" var="srcField">
                            <option value="${srcField.name}" <c:if test="${fn:toUpperCase(srcField.name) == fn:toUpperCase(etlField.srcFieldName)}">selected="selected"</c:if>>${srcField.comment}-${srcField.name}</option>
                        </c:forEach>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="doc-ipt-desFieldName-1" name="desFieldName">
                        <c:forEach items="${desFields}" var="desField">
                            <option value="${desField.name}" <c:if test="${fn:toUpperCase(desField.name) == fn:toUpperCase(etlField.desFieldName)}">selected="selected"</c:if>>${desField.comment}-${desField.name}</option>
                        </c:forEach>
                    </select>
                </div>
                <div>
