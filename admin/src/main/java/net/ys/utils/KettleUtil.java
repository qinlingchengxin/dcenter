...
public class KettleUtil {
    ...
    static Map<String, Job> jobMap = new ConcurrentHashMap<String, Job>();
    ...
    public static boolean genKtrFile(EtlEntity entity, EtlProject project, List<EtlField> fields, String transName) {
        try {
            ...
            String quotesLeft;
            String quotesRight;
            if ("MySql".equals(project.getCenterDbType())) {
                quotesLeft = "`";
                quotesRight = "`";
            } else if ("Oracle".equals(project.getCenterDbType())) {
                quotesLeft = "\"";
                quotesRight = "\"";
            } else {
                quotesLeft = "[";
            ...
            DatabaseMeta srcDbMeta = new DatabaseMeta("src_db", project.getCenterDbType(), "Native", project.getCenterDbIp(), project.getCenterDbName(), project.getCenterDbPort(), project.getCenterDbUsername(), project.getCenterDbPwd());
            ...
            DatabaseMeta desDbMeta = new DatabaseMeta("des_db", project.getBusDbType(), "Native", project.getBusDbIp(), project.getBusDbName(), project.getBusDbPort(), project.getBusDbUsername(), project.getBusDbPwd());
            properties = desDbMeta.getAttributes();
            if ("MySql".equals(project.getBusDbType())) {
                properties.setProperty("EXTRA_OPTION_MYSQL.characterEncoding", "utf8");
            } else if ("Oracle".equals(project.getBusDbType())) {
                properties.setProperty("EXTRA_OPTION_ORACLE.characterEncoding", "utf8");
            } else {
                properties.setProperty("EXTRA_OPTION_MSSQLNATIVE.characterEncoding", "utf8");
            }
            ...
            selectSql.append(" ORDER BY ").append(" SYS__DATETIME_STAMP ASC ");
            ...
            String transFilePath = SysConfig.ktrPath + transName + ".ktr";//转换生成的配置文件的路径
     ...
     * @throws Exception
     */
    public static void startJob(final String jobPath, String entityId) throws KettleException {
     ...
     * @throws Exception
     */
    public static void stopJob(String entityId) throws KettleException {
        Job jobTemp = jobMap.get(entityId);
        if (jobTemp != null && jobTemp.isAlive()) {
            KettleEnvironment.init();//初始化
            jobTemp.stopAll();
